...
"""
Performs Lexical analysis to identify input's type 
Argument: User input
Returns: tokens[]
"""
def lex(user_input):
    ...
    while pos < len(user_input):
        ...
        # identifiers
        identifier = ''
        while pos < len(user_input) and user_input[pos].isalnum():
            identifier += user_input[pos]
            pos += 1
        if len(identifier) > 0:
...
def typecheck_term(term):
    ...
    if isinstance(term, Functor):
        ...
        return
    raise InvalidInputError('Invalid term: {}.'.format(term))
...
def typecheck_formula(formula):
    ...
    if isinstance(formula, And):
        typecheck_formula(formula.formula1)
        typecheck_formula(formula.formula2)
        return
    ...
    if isinstance(formula, ForAll):
        if not isinstance(formula.variable, Variable):
            raise InvalidInputError('Invalid bound variable in FORALL quantifier: {}.'.format(formula.variable))
        typecheck_formula(formula.formula)
        return
    if isinstance(formula, ThereExists):
        if not isinstance(formula.variable, Variable):
            raise InvalidInputError('Invalid bound variable in exists quantifier: {}.'.format(formula.variable))
        typecheck_formula(formula.formula)
        return
    raise InvalidInputError('Invalid formula: {}.'.format(formula))
    ...
    while True:
        try:
            ...
            if len(tokens) > 0 and tokens[0] == 'axioms':
                ...
                formula = parse(tokens[1:])
                check_formula(formula)
                result = prover.prove_formula(axioms | set(lemmas.keys()), formula)
                if result:
